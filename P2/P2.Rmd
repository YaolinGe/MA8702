...
```{r INLA3, echo=FALSE, fig.width=12, fig.height=8, fig.cap="\\label{fig:INLA}Posterior marginals for selected effects"}
...
plot(inla.smarginal(res$marginals.fixed[[1]]),
     ...
     main="Posterior marginal for intercept")

plot(inla.smarginal(res$marginals.fixed[[2]]),
     xlab="beta", ylab="density",
     main="Posterior marginal for slope")
...
```{r GLMM1, echo=FALSE, fig.width=12, fig.height=4, fig.cap="\\label{fig:INLA}Posterior marginals for selected effects"}
library(INLA)
# Load dataset
data(Seeds)
...
Mention covariates which are 0-1 